...
abstract class CollectionTests {
	...
	void streamTest() {
		...
		Stream<Integer> concat = Stream.of( collection.stream().min(Integer::compare).get(), 
											collection.stream().max(Integer::compare).get());
		Integer array[] = {-5, 40};
		assertArrayEquals(array, concat.toArray());
